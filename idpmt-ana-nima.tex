...
\label{eq:laestim}
\end{align}
where $Q_g$ is the mean value of $g(x)$\footnote{The formulae of $Q_s$ and $Q_g$ together with their derivation can be found in \cite{me2}. } and $Q_s^\prime$ is the gain parameter returned by the minimizer.
...
One can see that the inside the $\sigma/Q$ = 25 -- 35 \% range the model extracts the gain with excellent accuracy. 
At the high\footnote{And unrealistic for modern PMTs.} value of $\sigma/Q$ = 45 \% there is a deviation from the true gain of $\sim$ 1.0 \% for $\mu = 5$.
...
\begin{figure}[!t]
\centering
\includegraphics[width=11.3cm, height=7.8cm]{figures/fit-1-b.pdf} \\[1.5ex]
\includegraphics[width=11.3cm, height=7.8cm]{figures/fit-3-b.pdf} %\\
